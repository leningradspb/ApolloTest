...
public class Channel {
    ...
    var bindings: [(event: String, ref: Int, callback: (Message?, Message) -> Void)]
    ...
    init(topic: String, params: [String: Any]?, socket: Socket) {
        ...
        self.on(ChannelEvent.reply) { [weak self] (sendedMessage, receiveMessage) in
            guard let strongSelf = self else { return }
            let replyEventName = strongSelf.replyEventName(receiveMessage.ref)
            let replyMessage = Message(ref: receiveMessage.ref,
                                       topic: receiveMessage.topic,
                                       event: replyEventName,
    ...
    @discardableResult
    public func onClose(_ callback: @escaping ((_ sendedMessage: Message?, _ receiveMessage: Message) -> Void)) -> Int {
    ...
    /// - return: The ref counter of the subscription
    @discardableResult
    public func onError(_ callback: @escaping ((_ sendedMessage: Message?, _ receiveMessage: Message) -> Void)) -> Int {
    ...
    @discardableResult
    public func on(_ event: String, callback: @escaping ((_ sendedMessage: Message?, _ receiveMessage: Message) -> Void)) -> Int {
        let ref = bindingRef
    ...
    func isMember(_ message: Message) -> Bool {
        ...
        if message.joinRef != nil, isLifecycleEvent {
            return true
        }
        else {
            self.socket?.logItems("channel", "dropping outdated message", message.topic, message.event, message.payload)
            return false
        }
    }
    ...
    func trigger(_ message: Message) {
        ...
        var sendedMessage: Message? = nil
        if let messageId = message.joinRef {
            sendedMessage = socket?.sentMessages[messageId]
        }
        self.bindings
            .filter( { return $0.event == message.event } )
            .forEach( { $0.callback(sendedMessage, handledMessage) } )
    }
