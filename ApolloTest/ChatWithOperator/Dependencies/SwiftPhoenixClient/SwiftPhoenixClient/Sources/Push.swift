...
public class Push {
    ...
    func send() {
        ...
        self.startTimeout()
        self.sent = true
        channel.socket?.push(body: [channel.topic, self.event],
                             payload: self.payload,
                             ref: self.ref,
                             messageId: channel.joinRef)
    }
    ...
    func startTimeout() {
        ...
        channel.on(refEvent) { [weak self] (_ sendedMessage: Message?, _ receiveMessage: Message) in
            guard let strongSelf = self else { return }

            strongSelf.cancelRefEvent()
            strongSelf.cancelTimeout()
            strongSelf.receivedMessage = receiveMessage
            
            /// Check if there is event a status available
            guard let status = receiveMessage.status else { return }
            strongSelf.matchReceive(status, message: receiveMessage)
            }
