...
public class Socket {
    ...
    /**
     Список отправленных сообщений с их идентификаторами.
     */
    private(set) var sentMessages:[String : Message] = [String : Message]()
    ...
    /**
     ...
     Возвращает уникальный идентификатор отправленного события,
     т.к. ответ сервера не типизирован и не позволяет его десериализовать без знания исходного запроса.
     
     - returns: Уникальный идентификатор отправленного события, позволяет смержить отправленное событие и его ответ сервера.
     */
    public func push(topic: String, event: String, payload:[String: Any]) -> String {
        let ref = makeRef()
        sentMessages[ref] = Message(ref: "", topic: topic, event: event, payload: payload, joinRef: ref)
        self.push(body: [topic, event], payload: payload, ref: "1", messageId: ref)
        return ref
    }
    
    public func push(body: [Any], payload:[String: Any], ref: String? = nil, messageId: String? = nil) {
        let callback: (() throws -> ()) = {
            var sendBody = body
            if let safeMessageId = messageId { sendBody.insert(safeMessageId, at: 0) }
//            if let safeRef = ref { sendBody.insert(safeRef, at: 0) }
            sendBody.insert("1", at: 0)
            sendBody.append(payload)
            let data = try JSONSerialization.data(withJSONObject: sendBody, options: [])
    ...
    fileprivate func onConnectionOpen() {
    ...
    fileprivate func onConnectionClosed() {
    ...
    fileprivate func onConnectionError(_ error: Error) {
        self.logItems("transport", error)
        self.onErrorCallbacks.forEach( { $0(error) } )
        self.triggerChannelError()
    }
    
    fileprivate func onConnectionMessage(_ rawMessage: String) {
        ...
        self.channels.forEach( { $0.trigger(message) } )
        ...
        if message.joinRef == pendingHeartbeatRef {
            self.logItems("received pending heartbeat")
            pendingHeartbeatRef = nil
        }
        // Удаление отправленного сообщения, после получения ответа. Необходимо для избегания утечки памяти.
        if let messageId = message.joinRef, sentMessages[messageId] != nil {
            sentMessages.removeValue(forKey: messageId)
        }
    }
    ...
    @objc func sendHeartbeat() {
        ...
        self.push(body: ["phoenix", ChannelEvent.heartbeat],
                  payload: [String : Any](),
                  ref: "1",
                  messageId: self.pendingHeartbeatRef)
        
        ChatErrorHandler.handleInfo(ChannelEvent.heartbeat)
    }
}
